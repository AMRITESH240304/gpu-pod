...
jobs_executed = meter.create_counter("gui_worker.jobs.executed", unit="1", description="Jobs by GUI worker")
job_duration = meter.create_histogram("gui_worker.job.duration", unit="s", description="Job duration (GUI worker)")
...
def _loop(log_callback, stats_callback):
    ...
    with tracer.start_as_current_span("gui_worker.register") as span:
        ...
        try:
            ...
            logger.info("gui worker registered", extra={"worker_id": WORKER_ID, "gpu": gpu_model})
    ...
    while WORKER_RUNNING:
        try:
            with tracer.start_as_current_span("gui_worker.heartbeat") as span:
...
def _execute_job(job: dict, log_callback):
    ...
    with tracer.start_as_current_span("gui_worker.execute_job") as span:
        ...
        try:
            ...
            logger.info("gui worker job completed", extra={"job_id": job_id, "duration_s": round(duration, 2)})
            ...
            logger.error("gui worker job failed", extra={"job_id": job_id, "error": str(e)})
            try:
                requests.post(f"{SERVER_URL}/result", json={
